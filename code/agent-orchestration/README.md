...
| 迁移 | Flyway（纯 SQL，双库通用）；Flyway 9.x 不识别 `jdbc:opengauss` 前缀，单独用 PostgreSQL 驱动经 wire 兼容连同库执行 |
...
迁移脚本：`src/main/resources/db/migration/`（`V1__init_schema.sql`、`V2__attempt_pending_result.sql`）。
...
mvn compile                                    # 编译
mvn test                                       # 全量测试(H2 + Flyway + mock)
mvn spring-boot:run                            # 本地启动(需 openGauss，端口 8080)
mvn spring-boot:run -Dspring-boot.run.profiles=mock   # mock 联调:mock Agent Core + mock AM
```

**Profile**：
- 默认：连 openGauss（`DB_URL`/`DB_USER`/`DB_PASSWORD` 可覆盖），真实 client 占位（`UnconfiguredAgentCoreClient`）；
- `mock`：`MockAgentCoreClient`（可配 SUCCEED/FAIL/TIMEOUT，异步推事件）+ `MockAgentManagementClient`；
- `test`：H2 PostgreSQL 兼容模式 + Flyway，关闭后台调度轮询（由测试显式驱动）。

...
> Session 与沙箱的设计——Orchestration 如何管理 session（每个 step = 一个 session）、Agent Core 如何在 session 内自行管理沙箱、工作流 / 聊天两种场景下的用法——见独立文档《[沙箱设计：生命周期、工作流与聊天场景](2026-06-05-sandbox-design.md)》。核心结论：**Orchestration 只感知 session，不感知沙箱；沙箱是 Agent Core 内部领域。** 本节的"进程可死 + resume 重载"对编排层透明。

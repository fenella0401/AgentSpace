...
- `GET /sessions/{id}/changes` 查看本次改动：不带参数返回改动文件列表（路径、变更类型、增删行数 + baseCommit/headCommit）；带 `path` 返回该文件的 unified diff。基于 workspace 内 git 工作区计算，供前端展示 diff、用户审查。超大/二进制文件按策略截断或标记。

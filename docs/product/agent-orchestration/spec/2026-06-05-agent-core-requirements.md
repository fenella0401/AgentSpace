...
- `GET /sessions/{id}/chat` 对话接口。建连参数：`prompt`、`resumeFromSessionRef`（可选）。SSE 流式返回 thinking / message / tool_use / tool_result / stdout / stderr / session.completed（含 summary、result、artifactRefs、commitSha、sessionRef）/ session.failed。
...
接收 prompt，按 `agentRuntime` 选择执行器启动代码 Agent，经 SSE 流式返回所有执行事件。对话过程中 agent 的代码改动负责 commit/push。对话上下文持久化落盘（键=sessionRef），执行进程退出后不丢，续聊时 `--resume` 重载恢复。外部对进程冷热无感知，续聊一律走 `GET /sessions/{id}/chat`，带 `resumeFromSessionRef`。
...
以用户为单位保留 session 数据：保留天数 ≥ 15 天 AND 用户 session 数 > 50 条，两个条件同时满足方可清理。

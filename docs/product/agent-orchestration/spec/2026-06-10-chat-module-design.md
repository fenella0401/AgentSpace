...
单页面：左侧会话列表（含「新建」），右侧当前对话。新建会话和查看/续聊历史会话共用同一布局——右侧对话区在「空白新对话」与「某历史会话」之间切换，不再分两个页面。
...
│ 🔍 搜索      │                                  │
│──────────────│   （对话消息区）                   │
│ ● 会话A  今天 │   用户消息 / Agent thinking /     │
│   会话B  昨天 │   工具调用 / Agent 回复 /          │
│   会话C  3天前│   改动文件卡片                     │
│   ...        │                                  │
│              │──────────────────────────────────│
│ [筛选: 全部▾] │  [输入框................] [发送]  │  输入区
...
2. 列表项显示标题（无标题时取首条消息摘要）、最近活跃时间、状态点（进行中/已完成/失败）；
3. 顶部「+ 新建」→ 右侧切到空白新对话（暂不创建 conversation，待首条消息发送时再建，见 §6#1）；
4. 顶部搜索按标题/内容检索；底部筛选器（预留：全部 / 聊天 / 工作流任务，见 §5）；
5. 列表项 hover 出现重命名、归档、删除；
6. 空状态：无会话时右侧直接是新对话引导。

### 2.2 右栏：当前对话

承载「新建」和「续聊」两种状态，交互一致：

1. **新对话**：右侧为空白对话 + 输入框。用户输入首条消息 → `POST /conversations`（若尚未创建）+ `POST /conversations/{id}/messages`，左栏顶部插入该会话并选中；
2. **历史会话**：点击左栏某项 → 右侧加载完整对话历史（`GET /conversations/{id}`），可直接在输入框续聊；
3. 发送消息后发起 SSE 订阅，流式返回 agent 执行事件，实时渲染；
4. 一轮结束（`session.completed`）展示结果摘要、改动文件卡片（点开看 diff）；
5. 继续输入 → 续聊，重复 3~4。
...
- **新旧切换**：右上「⋯」菜单含重命名、归档、删除（与左栏 hover 操作一致）。

...
>
> 核心约定：**两个核心接口**完成所有工作——`POST /sessions` 创建会话，`GET /sessions/{id}/sse` 进行对话。其余接口辅助。
...
| `GET /sessions/{id}/sse` | **核心** | SSE 对话——建连即开始/续接对话，流式返回所有执行事件 |
...
创建会话，传入本轮执行所需的全量配置。Agent Core 准备环境，返回 `sessionId` 和 SSE 对话地址。
...
## 2. 核心接口二：GET /sessions/{sessionId}/sse（SSE 对话）
...
| 无 repo 对话 | P2 | session 不传代码仓时，Agent Core 直接做 LLM 推理 + 搜索检索，不进沙箱，经 SSE 流式返回 |
| 沙箱内/外工具分流 | P2 | Agent Core 自行判断本轮是否需沙箱内工具（读写文件/执行命令），仅沙箱内工具才触发沙箱创建；沙箱外工具（搜索/检索）可在无沙箱状态直接调用 |
...
| 沙箱空闲流转（聊天场景） | P2 | 无代码仓 session 的沙箱按 NONE→WARM→FROZEN→RECLAIMED 自动流转 |
| 镜像治理 | P2 | 执行环境镜像受版本与来源管控，保证可复现、可审计 |

> **排期前提**：MVP 仅做工作流场景（聊天页延后）。工作流依赖所有 P0 + P1。P2 待排期时整体提级，沙箱冻结/解冻因工作流需要已在 P1。
...
Agent Core 在 session 初始化时，根据 `executorType` 选择对应的执行器实现。不同执行器的 skill/MCP 装配方式、prompt 格式、事件协议由 Agent Core 内部适配，对外接口（SSE 事件流）保持统一协议。
...
| # | 议题 | 说明 |
|---|---|---|
| 1 | SSE 续聊参数传递方式 | `resumeFromSessionRef` 通过 SSE 建连 query 参数还是首条事件传 |
| 2 | 终态事件双通道 | `session.completed` 在 SSE 和回调两通道——两边内容是否完全一致 |
| 3 | 沙箱冻结/解冻阈值 | Agent Core 自行决定 |
| 4 | 聊天场景 session 创建时机 | 无代码仓时惰性创建还是先建 session 再 SSE |
| 5 | 跨 session 代码仓并发 | 同 repo 多 session 并行修改的隔离/加锁 |
| 6 | session 配额与并发 | 按用户/项目限制活跃 session 数 |
| 7 | MCP server 跨 session 复用 | 健康检查与失效回收 |
...
>
> 核心约定：Agent Core 暴露 **session** 作为执行单元。外部初始化 session（传入 agent/skill/MCP 引用、代码仓、凭证），得到 `sessionId`。对话通过 **SSE 连接**进行——建连即开始一轮对话，流式返回 Agent 执行事件；断连后重连同一 SSE 地址即可续聊。**沙箱在 Agent Core 内部管理，外部不可见。**
...
Agent Core 的执行单元。初始化时传入全量配置（agent/skill/MCP 引用、代码仓、凭证、workspace 卷），Agent Core 在此上下文中管理沙箱与对话。
...
外部将 session 作为执行单元——创建时传入当前任务所需的配置（agent/skill/MCP 引用、代码仓、凭证），得 `sessionId` 后通过 SSE 连接进行对话。多个 step 或多轮对话通过多个 session 串联，文件传递靠代码仓 commit + checkout。

...
session 初始化的入参决定了启动时需要加载多少东西。入参越少，启动可以越快、沙箱可以越轻。Agent Core 应**根据入参自动选择合适的沙箱技术**，用最小成本满足隔离要求。

### 6.1 场景分级

| 场景 | 入参特征 | 需要做的事 | 启动性能要求 | 适合的沙箱 |
|---|---|---|---|---|
...
Agent Core 根据 session 入参选择沙箱实现，编排层不感知。当前需支持：
...
| 空闲时长 | 恢复方式 | 目标恢复时间 | 说明 |
|---|---|---|---|
| < 10 分钟 | 热恢复 | 毫秒级 | 进程仍存活或沙箱仍在 WARM 态，直接续聊，低延迟 |
| 10 分钟 ~ 1 小时 | 温恢复 | 秒级 | 沙箱已冻结（FROZEN），解冻后 `--resume` 重载上下文 |
| > 1 小时 | 冷恢复 | 十秒级 | 沙箱已回收（RECLAIMED），重新创建沙箱后 `--resume` 从持久化上下文冷启 |

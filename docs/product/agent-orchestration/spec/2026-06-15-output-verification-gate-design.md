...
| D3 | 不通过动作 | **可配置**：`SUSPEND`（转人工）/ `AUTO_RETRY`（自动续聊 N 次后转人工）/ `FAIL` | 由验收配置的 `onFail` 指定，默认 `SUSPEND` |
...
通过 harness 配置下发 Claude Code 的 hook（见 §8 harness 衔接）。关键 hook：
...
| 通用任务模块设计 | 增「Agent 任务准出验收」：task 收尾触发点、验收配置载体（缺省继承项目级默认）、裁决动作映射到 task 状态（见 §7）|
...
| 7 | Agent 任务收尾判定 | Agent 模式"对话完成"如何判定——纯靠用户「完成任务」动作，还是支持自动收尾条件（连续 N 轮无改动等），影响 gate 触发时机（见 §7.1）|

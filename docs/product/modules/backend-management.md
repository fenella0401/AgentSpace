...
在630版本中，节点模板不在新建团队空间时复用，也不会自动影响已有团队空间 Harness。

下个迭代计划支持：新建团队空间时选择某个系统级节点或 PDU 部门节点下的某个已发布模板，基于该模板初始化团队空间 Harness。为支持后续能力，630版本应保留模板来源节点、模板版本和模板内对象标识等可追溯信息。
...
- 插件发布时由 AgentSpace 后端按照 AgentCenter 格式要求打包为 AgentRuntime 可识别使用的 ZIP，并推送到当前用户所属且有权限的 AgentCenter 市场组织。
- 记录成员授权、模板保存、模板发布、插件保存、插件发布和权限拒绝审计。

### 2.4 不包括

- 新建、编辑、删除、移动或合并 PDU 部门节点。
- 在630版本中新建团队空间时选择节点模板初始化 Harness。
- 将节点模板自动同步到团队空间 Harness。
- 节点模板中的知识库、Agent.md 和环境变量配置。
- AgentCenter 市场中的插件审核、上架、下架和组织成员管理。
- AgentSpace 后端打包能力和 AgentCenter 的底层实现。
- 具体接口、数据库表和服务部署设计。
...
每个插件必须绑定当前节点下某个已发布模板版本，并从该模板版本中选择一个或多个 AgentFlow。插件发布时，AgentSpace 后端基于界面配置的 Harness 内容生成符合 AgentCenter 格式要求、可被 AgentRuntime 识别使用的 ZIP，并推送到 AgentCenter 市场。保存插件只更新草稿态配置，不产生市场版本。
...
| `PackageArtifact` | 基于某次插件发布配置和绑定模板版本生成的 ZIP 产物，可供 AgentRuntime 识别使用 |
...
本节以“二级部门节点管理员”为例说明部门节点管理员在后台管理面中的典型活动流。其他层级部门节点管理员遵循相同规则：一级至四级部门节点管理员可以管理本节点和直属下一层级节点；五级部门节点管理员只能管理本节点。

```mermaid
flowchart TD
    A[部门节点管理员进入 AgentSpace 配置中心] --> B[选择 PDU 部门管理域]
    B --> C[在部门树中选择目标部门节点]
    C --> D{是否具备目标节点管理权限}
    D -- 否 --> E[进入模板管理浏览态]
    E --> F[只读查看已发布模板清单和详情]
    F --> G[写操作由后端拒绝并记录权限拒绝审计]
    ...
    J --> J1[查看当前节点显式授权管理员]
    J1 --> J2[添加或撤销本节点/直属下级节点管理员]
    J2 --> M[服务端复核权限、节点层级和用户有效性]

    K --> K1[新建、编辑或保存节点模板草稿]
    K1 --> K2[Publish 模板生成已发布版本]
    K2 --> M

    L --> L1[新建或编辑节点插件]
    L1 --> L2[绑定当前节点已发布模板版本并选择 AgentFlow]
    L2 --> L3[保存草稿或发布到 AgentCenter 市场组织]
    L3 --> M

    M --> N{复核是否通过}
    N -- 否 --> O[拒绝操作，页面切换只读或保留未提交内容]
    N -- 是 --> P[写入成员/模板/插件结果]
    P --> Q[记录操作者、权限来源、节点、对象、动作和结果审计]
```

活动流说明：

1. 部门节点管理员先进入“PDU 部门”管理域，再从部门树选择目标节点。页面展示不是最终权限依据，所有写操作仍由服务端重新校验。
2. 当目标节点属于管理员可管理范围时，页面进入管理态，展示“成员管理 / 模板管理 / 插件管理”。管理员可以维护本节点，以及规则允许的直属下一层级节点。
3. 成员管理只展示目标节点的显式授权管理员，不展示因为上级节点继承而拥有管理能力的管理员。添加或撤销管理员时，需要再次校验操作者权限、目标节点层级和候选用户有效性。
4. 模板管理活动包括新建模板、编辑草稿、保存草稿和 Publish。Publish 成功后生成该模板的新已发布版本，供普通用户浏览和插件绑定使用。
5. 插件管理活动包括新建或编辑 Extension、绑定当前节点下的已发布模板版本、选择 AgentFlow、保存草稿和发布到 AgentCenter 市场组织。插件不能跨节点绑定模板或选择 AgentFlow。
6. 如果管理员选择了超出权限范围的节点，或权限在操作过程中被撤销，页面应切换到只读浏览态；后端必须拒绝保存、发布、新建版本、授权和撤销请求，并记录权限拒绝审计。
...
- 发布：对当前草稿版本进行校验，校验通过后由 AgentSpace 后端打包并推送到 AgentCenter 市场；发布成功后该插件版本号正式生效。
...
保存只更新草稿态，不生成 ZIP，不触发 AgentSpace 后端打包，也不推送 AgentCenter。已发布版本不可被覆盖；如需继续修改，应在同版本未发布草稿上编辑。
...
7. 系统按当前草稿版本配置、绑定模板版本和所选 AgentFlow 生成 README，并整理符合 AgentCenter 格式要求的打包内容。
8. AgentSpace 后端生成 AgentRuntime 可识别使用的 ZIP。
...
发布失败时保留当前草稿版本和上一成功发布版本，展示失败原因并允许修复后重试。用户取消组织选择时不执行打包，也不调用推送服务。同一版本号发布成功后不允许重复发布；需要产生新市场版本时，必须点击“新建版本”并填写新的版本号。
...
| 打包失败或超时 | 插件发布失败，不生成成功产物，保留草稿并允许重试 |
...
- Extension 打包是 AgentSpace 后端能力，用于按照 AgentCenter 格式要求将界面配置的 Harness 内容打包为 AgentRuntime 可识别使用的 ZIP；用户可在其他系统从 AgentCenter 下载插件，并配置到 AgentRuntime 中使用。
...
- 点击发布时先选择本人所属的单个 AgentCenter 市场组织，再由 AgentSpace 后端执行打包并推送到市场。
...
- 插件保存草稿、发布、新建版本，以及发布时生成 AgentRuntime 可识别使用的 ZIP 并推送到 AgentCenter 组织。
- 基础审计和异常处理。

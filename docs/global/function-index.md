...
| F-008 | 团队空间 Harness CICD 流水线配置与任务运行 | AgentSpace / Harness CICD 流水线与任务运行 | 草稿 | `docs/function-design/F-008-team-harness-cicd-pipeline-configuration-and-runtime.md` | 支持 Harness CICD 流水线结构化配置、画布、测试发布，并通过开始作业入口创建任务、运行 Run 状态机、维护运行清单和节点 Agent 会话关联。 |

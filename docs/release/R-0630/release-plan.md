...
| F-008 | 团队空间 Harness CICD 流水线配置与任务运行 | `docs/function-design/F-008-team-harness-cicd-pipeline-configuration-and-runtime.md` | R-0630 核心功能，支持 Harness CICD 流水线结构化配置、画布、测试发布，通过开始作业入口选择流水线并创建任务、Run 状态机、运行清单、人工确认、取消重试、Hook 启动 Harness CICD 流水线，以及每个 Agent 节点自动创建 Agent 会话。 |
